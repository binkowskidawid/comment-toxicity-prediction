...
from ..config import LABELS, MAX_FEATURES
...
def format_comment_results(comment: str, results: np.ndarray) -> str:
    ...
    for i, label in enumerate(LABELS):
        toxicity_level = (
            "HIGH" if results[i] > 0.5 else "MEDIUM" if results[i] > 0.2 else "LOW"
        )
...
def batch_analyze_comments(
    comments: list[str], model: Any, vectorizer: Any
) -> list[np.ndarray]:
    """
...
def get_labels_info() -> dict:
    ...
    return {"labels": LABELS, "descriptions": labels_info, "count": len(LABELS)}
